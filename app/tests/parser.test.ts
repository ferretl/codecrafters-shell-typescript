...
describe('redirects', () => {
  test('parses > as stdout overwrite', () => {
    ...
    expect(result.stdout).toEqual(O.some({ path: 'out.txt', mode: 'overwrite' }));
    expect(result.stderr).toEqual(O.none);
  });
  ...
  test('parses 2> as stderr overwrite', () => {
    ...
    expect(result.stderr).toEqual(O.some({ path: 'err.txt', mode: 'overwrite' }));
    expect(result.stdout).toEqual(O.none);
  });
...
describe('parse errors', () => {
  test.each(['>', '1>', '>>', '1>>', '2>', '2>>'])(
    "rejects a trailing '%s' with no target",
    (op) => {
      expectLeft(
        parser(`echo hi ${op}`),
